...
func (r *AtsEvaluationRepository) Create(eval AtsEvaluation) error {
	query := `
...
func (r *AtsEvaluationRepository) GetByID(id string) (AtsEvaluation, error) {
	query := `
	...
	if err != nil {
		return AtsEvaluation{}, err
	}

	return eval, nil
}

func (r *AtsEvaluationRepository) GetByResumeID(resumeID string) ([]AtsEvaluation, error) {
	query := `
		SELECT id, resume_id, job_id, score, summary, details, raw_response, breakdown_keyword_match, breakdown_technical, breakdown_experience, breakdown_impact, breakdown_readability, matched_keywords, missing_keywords, recommendations, created_at
		FROM ats_evaluations
		WHERE resume_id = ?
