...
func (s *OptimizationServices) Optimize(userID, resumeID, jobID string) (OptimizeResponse, error) {
	resume, err := s.ResumeRepo.GetByID(resumeID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return OptimizeResponse{}, errors.New("currículo não encontrado")
		}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return OptimizeResponse{}, errors.New("vaga não encontrada")
		}
		return OptimizeResponse{}, err
	}
...
func (s *OptimizationServices) GetByResumeID(userID, resumeID string) ([]OptimizeSummaryResponse, error) {
	resume, err := s.ResumeRepo.GetByID(resumeID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	result := make([]OptimizeSummaryResponse, 0, len(opts))
	for _, opt := range opts {
		result = append(result, OptimizeSummaryResponse{
			ID:        opt.ID.String(),
...
func (s *OptimizationServices) GetByIDPublic(optimizationID string) (OptimizeResponse, error) {
	opt, err := s.OptRepo.GetByID(optimizationID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return OptimizeResponse{}, errors.New("otimização não encontrada")
		}
		return OptimizeResponse{}, err
	}
...
func (s *OptimizationServices) GetByID(userID, optimizationID string) (OptimizeResponse, error) {
	opt, err := s.OptRepo.GetByID(optimizationID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return OptimizeResponse{}, errors.New("otimização não encontrada")
